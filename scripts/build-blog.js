  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (inFrontmatter) {
      ...
      if (line.includes(':')) {
        ...
        if (cleanKey === 'date') {
          frontmatter.date = value; // Use date field directly
...
function formatDate(dateString) {
  ...
  // Handle DD/MM/YYYY format (e.g., "31/08/2025")
  ...
  if (ddmmyyyyMatch) {
    ...
    return `${monthNames[month]} ${dayWithSuffix}, ${year}`;
  }

  // If it's already in readable format, use as-is
  if (dateString.match(/\w+\s+\d+\w*\s+\d{4}/)) {
    return dateString;
  }
